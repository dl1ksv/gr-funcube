...
fcd_control_impl::fcd_control_impl()
    : gr::block("fcd_control", gr::io_signature::make(0, 0, 0),
...
void fcd_control_impl::set_freq(float freq) {
  ...
  if (aucBuf[0] == FCD_HID_CMD_SET_FREQUENCY_HZ && aucBuf[1] == 1) {
    ...
    GR_LOG_INFO(d_logger,
                boost::format("Set Frequency to %1% Hz failed") % nfreq);
  }
}
...
void fcd_control_impl::set_lna_gain(float gain) {
  ...
  if (aucBuf[0] == FCD_HID_CMD_SET_LNA_GAIN) {
    ...
    GR_LOG_ERROR(
        d_logger,
        boost::format(
            "Failed to modify LNA gain. Result of transaction: %1%,%2%") %
            aucBuf[0] % aucBuf[1]);
  }
}
...
void fcd_control_impl::set_mixer_gain(float gain) {
  ...
  if (aucBuf[0] == FCD_HID_CMD_SET_MIXER_GAIN) {
    ...
    GR_LOG_ERROR(
        d_logger,
        boost::format(
            "Failed to modify Mixer gain. Result of transaction: %1%,%2%") %
            aucBuf[0] % aucBuf[1]);
  }
}
...
void fcd_control_impl::set_dc_corr(double _dci, double _dcq) {
  union {
    ...
    struct {
      signed short
          dci; // equivalent of qint16 which should be 16 bit everywhere
      signed short dcq;
    };
  } dcinfo;
  ...
  if (aucBuf[0] == FCD_CMD_APP_SET_DC_CORR) {
    ...
    GR_LOG_ERROR(d_logger, boost::format("Failed to set DC offset correction. "
